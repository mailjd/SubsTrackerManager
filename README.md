...
> **v3.3.18 表格保存根因修复**：修复“清空订阅类型被旧值回填”和“手动表格编辑过去日期触发隐式续期”。前后端均比对提交的实际字段值，不再只核对成功标记/字段名/后端已改写对象。增加保存中防重入、活动编辑器暂存、前后端版本校验和不包含单格内容的保存诊断。保留现有模板存储键。
>
> **验证范围与部署说明**：详见 [TABLE_SAVE_AUDIT_3.3.18.md](TABLE_SAVE_AUDIT_3.3.18.md)。本版修复了会阻断既有部署流程的一条过时提醒断言；没有删除或跳过原有测试。实际测试结果位于 `tests/regression/results/`。线上 Cloudflare 部署与真实用户数据未在本环境中访问。


> v3.3.17 表格持久化强校验修正：表格保存后必须进行实际存储回读校验，只有 KV 或 D1 当前快照确认本次字段已写入才返回成功；读取订阅时自动以 updatedAt 较新的 D1 当前快照覆盖 Cloudflare KV 边缘旧快照，避免“保存成功后刷新又回旧值”；更新流程也改为先读取 KV/D1 中较新的快照再合并，防止连续编辑因 KV 最终一致性读到旧记录而覆盖前次修改。

> v3.3.16 表格实际保存修正：订阅记录表格改用专用字段级 PATCH 保存接口，只提交本次编辑字段并由服务端基于数据库现值合并；修复提醒规则同步时 Cloudflare KV 最终一致性可能回读旧记录、再次写回从而覆盖刚保存单格修改的问题；服务端返回 saved=true 与已确认字段后前端才显示保存成功。

> v3.3.15 表格保存与模板持久化修正：表格单格保存成功后直接采用服务器返回的新记录更新当前列表，并增加短时写后读保护，避免 Cloudflare KV 最终一致性导致刚保存又显示旧值；订阅记录与 Database 显示模板同时持久化到 SUBSCRIPTIONS_KV，正常工具更新/重新发布后不会只依赖浏览器 localStorage。
