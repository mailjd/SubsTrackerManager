 ...
 * @returns {Array}
 */
async function syncSubscriptionMenuOptions(env, subscription) {
  const tasks = [];
...
function normalizeSubscriptionUsers(value) {
 ...
 */
async function createSubscription(subscription, env, options = {}) {
  try {
    ...
    const newSubscription = {
      ...
      endOfMonth: isSinglePeriod ? false : endOfMonthFlag,
    ...
    if (newSubscription.account || newSubscription.accountSerial) {
      if (newSubscription.account && newSubscription.accountSerial) {
        const accountCheck = await validatePair(env, newSubscription.accountSerial, newSubscription.account);
        if (!accountCheck.ok) {
...
async function updateSubscription(id, subscription, env) {
  try {
    ...
    const effectivePeriodUnit = subscription.periodUnit || existing.periodUnit || 'month';
    const isSinglePeriod = effectivePeriodUnit === 'single';
    let useLunar = !isSinglePeriod && !!subscription.useLunar;
    if (useLunar) {
      const expiryParts = getTimezoneDateParts(expiryDate, timezone);
    ...
    const merged = {
      ...
      points:
        subscription.points !== undefined
          ? (subscription.points === '' || subscription.points === null || !Number.isFinite(Number(subscription.points))
              ? null
              : Math.max(0, Number(subscription.points)))
          : (existing.points !== undefined ? existing.points : null),
      account:
        subscription.account !== undefined
          ? String(subscription.account || '').trim()
          : existing.account || '',
      accountSerial:
        subscription.accountSerial !== undefined
          ? String(subscription.accountSerial || '').trim()
          : existing.accountSerial || '',
      users:
...
async function deleteSubscription(id, env) {
  try {
    ...
    await recordSubscriptionDelete(env, existing);
...
async function manualRenewSubscription(id, env, options = {}) {
  try {
    ...
    if (subscription.periodUnit === 'single') {
      return { success: false, message: '单次订阅不支持续订，请编辑到期日期或改为周期订阅' };
    }
...
async function deletePaymentRecord(subscriptionId, paymentId, env) {
  try {
    ...
    await subRepo.save(env, updated);
    await recordSubscriptionChange(env, 'payment_delete', updated, { paymentId });
...
async function updatePaymentRecord(subscriptionId, paymentId, paymentData, env) {
  try {
    ...
    await subRepo.save(env, updated);
    await recordSubscriptionChange(env, 'payment_update', updated, { paymentId });
...
async function toggleSubscriptionStatus(id, isActive, env) {
  try {
    ...
    await subRepo.save(env, updated);
    await recordSubscriptionChange(env, 'status_change', updated, { isActive: !!isActive });
