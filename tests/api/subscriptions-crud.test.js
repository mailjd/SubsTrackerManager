...
describe('订阅 CRUD', () => {
  it('创建订阅成功并写入默认提醒规则', async () => {
    ...
    expect(body.subscription.users).toBe('张三,李四,王五');
    const stored = await subRepo.getById(env, body.subscription.id);
    expect(stored.users).toBe('张三,李四,王五');
  ...
  it('更新订阅名称', async () => {
    ...
    expect(got.users).toBe('Alice,Bob');
  });
